...
async function getCategoury(){
    let apiMeal = await fetch(`https://www.themealdb.com/api/json/v1/1/categories.php`);
    let response = await apiMeal.json();
    let finalRespos= response.categories;
    console.log(finalRespos);
    let cartona='';
     for (let i = 0; i < finalRespos.length; i++){
        cartona+=`<div class="col-md-4">
        <div class="item ">
            <img src="${finalRespos[i].strCategoryThumb}" alt="">
            <div class="imgCaption ">
                <h5 class='fw-bold fa-1x'>${finalRespos[i].strCategory}</h5>
            </div>
        </div>
    </div>`
        
     }
     $('.categorySection .row').html(cartona)
     // select category
     $('.col-md-4 h5').click(async function(e){

        let catalogSelect= $(e.target).text();
        let apiMeal = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${catalogSelect}`);
    let response = await apiMeal.json();
    let finalRespos= response.meals; 
    console.log(finalRespos);
    let cartona = '';
    $(".mealOfCategorySelected").css('display','block')
    ...
    for (let i = 5; i < finalRespos.length; i++) {
        ...
        <div class="imgCaption ">
            <h5 class="headItem fw-bolder">${finalRespos[i].strMeal}</h5>
        </div>
        </div>
   ...
   $('.mealOfCategorySelected .row').html(cartona)
   ...
   $('.col-md-4 h5').click(async function (e) {
    ...
    for (let i = 0; i < finalresult.length; i++) {
        ...
        $('.mealOfCategorySelected ').css('display', 'none')
        $('.categorySection ').css('display', 'none')
    ...
    $('.mealOfDescraption .row').html(cartona)
